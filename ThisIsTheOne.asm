...
Check0:
	...
	OUT		SSEG1
	XOR		c7FFF
	JZERO	Start	; if (distance == 7FFF) continue
	
	LOAD	CurrDist
	SUB		MinDist
	JNEG	Start	; if (distance < MinDist) continue
	
	LOAD 	CurrDist
	SUB		MaxDist
	JPOS	Start	; if (distance > MaxDist) continue
...
Check2:
	...
	LOAD	CurrDist
	SUB		MinDist
	JNEG	TurnFaceWall	; if (distance < MinDist) continue
...
;Calculate and store max and min distances
ADD		error
STORE	DistLMax
LOAD	DistL
SUB		error
...
SUB		tLastR
ADDI	-10
JNEG	Adjustment  ; Move ;Skip moving right check if has moved right in last few seconds
...
;If distread-distLmin < 0, jneg to turn right slightly (>355deg)
;Skip1:
;IN		TIMER
;SUB		tLastR
;ADDI	-10
...
;Corner code - advance ~240 mm using odometer, timer or speed*time
Corner:
;Slow down initially`
LOAD	Zero
STORE	DVel
...
;End code - re-orient, beep
Endcode: 
;Stop movement
LOAD	Zero
STORE	DVel

;Re-orient to starting orientation
;LOAD	Zero
LOAD	DTheta
SUB 	ORIENT0
CALL	Mod360
...
CLI 	&B1111 ;disable interrupts
;Can include other output/actions here
;Display debug reason (reason for death/exit)
LOAD	DREASON
OUT		LCD
...
;***********************************************************************MVMT CODE*****************************************
;Defined variables
DistL:		DW	 &H0000  ;Passed in from orientation phase
DistLMax: 	DW	 &H0000
DistLMin:  	DW	 &H0000
DistRead:  	DW	 &H0000
...
CurrDist:	DW 0
TurnAngle:	DW 0
MinDist:	DW &H1090 ;1090
MaxDist:	DW &H1200 ;1200
Check:		DW 0
